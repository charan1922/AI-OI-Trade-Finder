...
- `init/08-expiry-roll-and-privacy-release-2026-07-26.md` — the 26 July production release in plain English: the expiry-week contract roll (first fires Monday 27 July), the AI-config bug that could have switched off the stop-loss, three read-only-viewer position leaks, the four damaged-download checks, restart recovery, what was measured versus reasoned, a mistake caught in review, and the seven remaining gaps. **LIVE on `prod` at `88ede7f`.** Addendum (27 July): the roll replayed against the real contract file before the open (208/210 names roll to 25 August; EXIDEIND and NUVAMA are correctly refused), measured costs showing why the proposed hand-built expiry calendar was declined, and follow-up PR #27 (label honesty, commentary stored-visibility, and the three spread ceilings that shared one name) with proof it is not deployed.
