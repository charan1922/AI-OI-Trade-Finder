...
export const COMMENTARY_HARD_RULES = [
  ...
  '  "extended" → "already moved a lot —',
  '  entering now is chasing". "tfBreakout fakeout-risk" → "this breakout smells fake: the morning floor',
  '  already broke once — be cautious". A number appears ONLY when it is the instruction itself:',
...
export const COMMENTARY_SYSTEM = [
  ...
  '',
  'THE BAR for "TRADE NOW": every suggestion already passed the deterministic TF selector: fresh TF',
  'R-Factor accumulation over 30 minutes, a real directional move, an opening-',
  'range breakout, a tradeable options-premium pool, and less than 2% extension since 09:45. Never',
  're-veto one of those measured gates using App R-Factor, App OI, VWAP, tfBreakout grade, sector,',
  'chaotic-open, or moveProfile — those fields are context only on this TF path.',
  '- Prefer moveProfile "fresh" between otherwise-equal picks. "spent"/"fading" is a caution, not an',
  '  automatic rejection, because the TF ablation found a separate freshness hard gate slightly harmful.',
  '- Prefer a clean consolidation when present, but orBreakout already proves the required breakout.',
  ...
  '`tfSelection` is the PRIMARY selector provenance. TradeFinder R-Factor measures the SIZE',
  'of big-money participation — how much large capital is building there — and is DIRECTIONLESS. A top',
  'rank does NOT mean "going up": on 2026-08-07 the #1 name on their board was DOWN 6.39%. Use it only',
  'to establish participation; direction is TF price change confirmed by opening-range breakout.',
  '`suggestions` contains only names with matching TF-selection provenance.',
  'On "extended": extension from the open is descriptive. The scanner already enforces the measured',
  'since-09:45 chase band. Extended alone is NOT a veto: call it a late',
  ...
  'Fresh autonomous suggestions run only in the configured morning window (default 09:40–11:00).',
  'Outside it, manage existing positions only; never invent an all-day entry permission.',
...
function trimForPrompt(r: SuggestResponse): unknown {
  ...
  return {
    ...
    suggestions: suggestions.map((s) => ({
      ...
      // Evidence/caution factors — Supertrend is intentionally excluded from
      // Auto Trade and Trade Commentary by operator instruction.
      factors: s.factors && {
