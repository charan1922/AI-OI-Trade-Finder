...
import { selectSectorPromotions } from './sector-signal';
import type {
  ActiveSectorSignal,
  FeedPicks,
  PriorityFeed,
...
function indexFeeds(feedPicks: FeedPicks): Map<string, FeedIndexEntry> {
  ...
  for (const source of FEED_ORDER) {
    for (const pick of feedPicks[source] ?? []) {
      ...
      entry.feedReturns[source] = pick.retPct;
...
/** Feed candidates flattened & deduped, ordered best-rank-first (for promotions). */
function rankedRemaining(feedPicks: FeedPicks): { symbol: string; sector: string; retPct: number | null }[] {
  const best = new Map<string, { symbol: string; sector: string; retPct: number | null; rank: number }>();
  for (const source of FEED_ORDER) {
    for (const pick of feedPicks[source] ?? []) {
      if (!pick.symbol) continue;
      const cur = best.get(pick.symbol);
      if (!cur || pick.eligibleRank < cur.rank) {
        best.set(pick.symbol, {
          symbol: pick.symbol,
          sector: pick.sector ?? cur?.sector ?? '',
          retPct: pick.retPct,
...
export function buildPriorityPlan(input: BuildPriorityPlanInput): PriorityPlan {
  ...
  // ── Tier 0: continuity — never capped ─────────────────────────────────────
  ...
  const baseTier1 = selectRoundRobinCandidates(feedPicks, perFeedLimit, baseSlots);
  ...
  const sectorPromoted =
    sectorEnabled && sectorSlots > 0
      ? selectSectorPromotions({
          remainingFeedCandidates: rankedRemaining(feedPicks).map((r) => ({
            symbol: r.symbol,
            sector: r.sector,
            source: 'nse-oi' as PriorityFeed, // source is unused by the promoter
            eligibleRank: 0,
            retPct: r.retPct,
          })),
          activeSectors,
  ...
  if (tier1.length < maxUniqueTier1) {
    const fillers = selectRoundRobinCandidates(feedPicks, perFeedLimit, maxUniqueTier1 - tier1.length, new Set(tier1));
