...
export function GET(): Response {
  ...
  if (!p.credentialsConfigured) {
    fyers = { status: 'down', detail: 'Not configured (FYERS_* env vars)', tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  } else if (!p.started) {
    fyers = { status: 'down', detail: 'Poller not started', tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  } else if (lc && lc.skipped === 'no-credentials') {
    fyers = { status: 'down', detail: 'Credentials rejected on last cycle', tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  } else if (!marketOpen) {
    fyers = { status: 'idle', detail: 'Market closed — poller sleeping between 5-min ticks', tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  } else if (lc && lc.symbolsProcessed > 0 && lc.errors.length === 0) {
    fyers = { status: 'ok', detail: `Last cycle: ${lc.symbolsProcessed}/${lc.universeSize} symbols, ${lc.futBars + lc.eqBars} bars`, tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  } else if (lc && lc.symbolsProcessed === 0 && lc.errors.length > 0) {
    fyers = { status: 'down', detail: `Last cycle failed: ${lc.errors[0]?.message ?? 'unknown'}`, tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  } else if (lc && lc.errors.length > 0) {
    fyers = { status: 'warn', detail: `Last cycle had ${lc.errors.length} error(s), ${lc.symbolsProcessed} ok`, tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  } else {
    fyers = { status: 'warn', detail: 'Waiting for first cycle', tokenExpiresAt: p.token.expiresAt, lastCycle: lc };
  }
  ...
  if (nseCache.feedsCached === 0 && nseCache.lastSuccessAt === 0) {
    nse = { status: 'idle', detail: 'No NSE feed requested yet this session', lastSuccessAt: 0 };
  } else if (nseAge != null && nseAge < NSE_FRESH_MS) {
    nse = { status: 'ok', detail: 'Feeds fetching normally', lastSuccessAt: nseCache.lastSuccessAt };
  } else if (nseCache.lastError) {
    nse = { status: 'warn', detail: `Last fetch failed: ${nseCache.lastError.slice(0, 80)}`, lastSuccessAt: nseCache.lastSuccessAt };
  } else {
    nse = { status: 'idle', detail: 'No recent NSE fetch (idle)', lastSuccessAt: nseCache.lastSuccessAt };
  }

  return NextResponse.json({ ok: true, ts: new Date(now).toISOString(), marketOpen, services: { dhan, fyers, nse } });
}
